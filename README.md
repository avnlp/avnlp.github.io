...
|------------------|-------------------------------------------|-------------------------------------------|-----------------------------------------|-----------------------------------------|-----------------------------------------|
| **Indexes**      | Supports only dense vectors, optimized for approximate nearest neighbor (ANN) search. Sparse vectors not supported | Supports both sparse and dense vectors, using HNSW for dense and BM25 for sparse indexing | Supports only dense vectors with flat embeddings, optimized for in-memory search | Supports both sparse and dense vectors, using IVF for dense indexing and BM25 for sparse retrieval | Supports both sparse and dense vectors, using HNSW for dense and hybrid search mechanisms for sparse |
| **Hybrid Search** | Single sparse-dense index. Requires both sparse and dense query vectors | BM25 + vector search with `alpha` parameter for balance | Not supported | BM25 + vector search using hybrid query modes | BM25 + ANN search with structured filtering |
| **Partition**    | Uses namespaces to partition records. Queries limited to one namespace | Uses tenants for isolation. Queries limited to a tenant | Uses collections as namespaces. Queries directed to a collection | Uses partitions to separate data. Queries limited to a partition | Uses named collections for data separation. Queries filtered within collections |
| **Semantic Search** | Finds similar content using vector proximity. Supports metadata filtering | Vector-based retrieval. Results based on embedding similarity | Stores and retrieves vector embeddings for similarity search | Uses IVF, HNSW, and ANNOY for efficient vector retrieval | Real-time vector similarity search with contextual relevance |
| **Metadata Filtering** | Dictionary-based metadata filtering attached to vectors | GraphQL-based filtering with hierarchical queries | Key-value filtering using Pythonic expressions | SQL-like filtering with structured metadata fields | Payload-based filtering with structured metadata |
